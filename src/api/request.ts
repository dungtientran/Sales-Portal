...
const VITE_PROD_MIGRATE_URL = import.meta.env.VITE_PROD_MIGRATE_URL as string;

export const axiosInstance = axios.create({
  // timeout: 6000,
  baseURL: VITE_PROD_MIGRATE_URL,
});
  ...
  async error => {
    ...
    if (error.response?.data.message == TOKEN_EXPIRED_MESSAGE) {
      try {
        ...
        const res = await axiosInstance.post('/user/sale/refresh_token');
